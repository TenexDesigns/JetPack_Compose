 ...
 ******* IN ROW THE ABOVE TWO ARE DIFFFFERRENT 
 // In row ,it HorizontalArrangment and VerticalAlignment
 IN ROW 
 HorizontalArrangment - Deals with how items are spread out or arrangaded on the horizontal axis. The items can either be arranged in a spaceBetweeen or spaceEvenly or spaceAroud or center or start or end of the vertical axix.
 VerticalAlignment  - Deals with where the row with the items is put on  the verticla axix . Either top ,bottom or center.
...
HERE IS AN EXAMPLE OF A SURFACE
 ...
 column(modifier = Modifier.fillMaxSize()){
  ...
  //This is the callable surface that will populate your cloumn when called in a column layout
  @Composable
fun ColumnScope.custom(weight:Float,color: Color){
  ...
  //This is the callable surface that will populate your row when called in a column layout
  
  @Composable
fun RowScope.custom(weight:Float,color: Color){
...
THEN THIS IS HOW YOU CALL THE SURFCA CLLLABLE METHODS
  
   column(modifier = Modifier.fillMaxSize()){
    
                        custom(weight = 2f, color = MaterialTheme.colors.secondary)
                        custom(weight = 2f, color = MaterialTheme.colors.primary)
   }
